...
class StudyIntent {
  const StudyIntent(this.ask, this.subject, {this.facet, this.terms = const []});
  final StudyAsk ask;
...
class StudyIntentParser {
  static StudyIntent parse(String raw) {
    ...
    final rules = <(RegExp, StudyAsk)>[
      ...
      (
        RegExp(r'^(?:define|what\s+is(?:\s+meant\s+by)?)\s+', caseSensitive: false),
        StudyAsk.definition,
      ),
      (
      ...
      (
        RegExp(r'^(?:compare|differentiate|distinguish)\s+', caseSensitive: false),
        StudyAsk.comparison,
      ),
    ];
